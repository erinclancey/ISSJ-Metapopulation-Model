...
prop.sd <- c(0.01, 0.5, 0.5, 0.01, 0.1, 0.1, 0.1, 0.01)
  ...
  for (i in 2:iter) {
    ...
    if (log (runif (1)) < log.accept.prob & 
                                  eta.new<0.3 & 
                                  theta_oak.new>20 & theta_oak.new<30 &
                                  theta_pine.new>20 & theta_pine.new<30 &
                                  gamma.new<0.3 & 
                                  SDa.new>0.5 & SDa.new<2.0 & 
                                  SDe.new>0.5 & SDe.new<1.5 & 
                                  SDs.new>0.5 & SDs.new<1.5) {
      eta[i]=eta.new
...
theta_oak = runif(1, 21,23)
theta_pine = runif(1, 24,26)
gamma = runif(1, 0.01, 0.1)
SDa = runif(1, 1,2)
SDe = runif(1,0.5,1)
SDs = runif(1,0.5,1)
m=runif(1,0.05, 0.15)

results <- Metro_Hast(eta, theta_oak, theta_pine, gamma, SDa, SDe, SDs, m, iter=10, Z_P_M, Z_P_F, Z_O_M, Z_O_F)
results.post <- do.call(cbind, results)
args = commandArgs(trailingOnly=TRUE)
filename <- sprintf("Results_New_%s.csv", args[1])
