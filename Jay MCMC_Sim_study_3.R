...
for (i in 1:nrow(estimates)) {
  ...
  if (class(sim_data) != "try-error") {
    ...
    Z_O_M <- sim_data[[11]]
    Z_O_F <- sim_data[[12]]
    results <-with(estimates, Metro_Hast(eta[i], theta_oak[i], theta_pine[i], gamma[i], SDa[i], SDe[i], SDs[i],m[i], 
                                       iter=3, Z_P_M, Z_P_F, Z_O_M, Z_O_F))
