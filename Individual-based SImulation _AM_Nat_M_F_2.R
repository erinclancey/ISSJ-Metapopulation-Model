...
SDe = 0.7 # environmental standard deviation
SDs = 0.7 # segregational standard deviation
sex = 1 # genetic factor causing sexual dimorphism
eta = 0.2 # strength of habitat preference
m = 0.1  # max prob of migration
theta_pine = 25.2 # ecological optimum in pine
theta_oak = 23.2 # ecological optimum in oak
gamma = 0.005 # strength of selection
...
for (r in 1:R){
   ...
   N_pine_F[r] <- length(Z_pine_f[,1])
   N_pine_M[r] <- length(Z_pine_m[,1])
   N_oak_F[r] <- length(Z_oak_f[,1])
   N_oak_M[r] <- length(Z_oak_m[,1])
   ...
   pooled_var[r] = 1/(N_pine_F[r]+N_pine_M[r]+N_oak_F[r]+N_oak_M[r] - 4) * ((N_pine_F[r]-1)*Vp_pine_F[r] + 
                                                                              (N_pine_M[r]-1)*Vp_pine_M[r] +
                                                                              (N_oak_F[r]-1)*Vp_oak_F[r] +
                                                                              (N_oak_M[r]-1)*Vp_oak_M[r] )
...
colors <- c("Zbar_pine_M" = "skyblue4","Zbar_oak_M" = "tomato4","Zbar_pine_F" = "skyblue3","Zbar_oak_F" = "tomato2")
p <- ggplot(Zbar, aes(x = Gen)) + theme_minimal()+ ggtitle("A")+ theme(axis.title.x = element_blank())+ theme(text = element_text(size = 12)) 
p <- p + annotate("rect", xmin = -Inf, xmax = Inf,ymin = 22.53915, ymax = 22.86291, alpha = .1) + 
          annotate("rect", xmin = -Inf, xmax = Inf,ymin = 24.62476, ymax = 24.93638, alpha = .1)+
          annotate("rect", xmin = -Inf, xmax = Inf,ymin = 23.22039, ymax = 23.77606, alpha = .1)+
         annotate("rect", xmin = -Inf, xmax = Inf,ymin = 25.33897, ymax = 25.8095, alpha = .1)
...
p <- p + geom_line(aes(y = Zbar_oak_F, color="Zbar_oak_F")) + labs(color = "") + 
  scale_color_manual(values = colors, labels = c("Pine Male","Oak Male", "Pine Female", "Oak Female"))+ 
...
p <- p + geom_hline(yintercept = 23.49823, color="skyblue3", linetype=2)
p <- p + geom_hline(yintercept = 22.70103, color="tomato2", linetype=2)
# p <- p + geom_hline(yintercept = theta_pine, linetype = 1, color="black", size=0.5)
# p <- p + geom_hline(yintercept = theta_oak, linetype = 1, color="black", size=0.5)
p <- p + scale_x_continuous() + scale_y_continuous(name = "$\\bar{z}$", breaks = seq(18,32,0.5), limits = c(22, 26))
A <- p
plot(A)


B <- ggplot(data.frame(Gen,pooled_var), x=Gen, y=pooled_var) + theme_minimal() + ggtitle("B") + theme(axis.title.x = element_blank())+ theme(text = element_text(size = 12))
B <- B + annotate("rect", xmin = -Inf, xmax = Inf, ymin = 1.159414, ymax = 1.467943, alpha = .1)
B <- B + geom_line(aes(x=Gen, y=pooled_var, color="pooled_var"))+ labs(color = "") + scale_color_manual(values = "black", labels = c("Total Pop."))
B <- B + geom_hline(yintercept = 1.3, color="black", linetype=2)
B <- B + scale_x_continuous() + scale_y_continuous(name = "$\\sigma^2_{pooled}$", breaks = seq(1,2,0.1), limits = c(1.0,1.8))
plot(B)



x.grob <- textGrob("Generation", gp=gpar(fontsize=13))
plot <- plot_grid(A, B, ncol = 1, nrow = 2, rel_heights=c(1.5,1))
