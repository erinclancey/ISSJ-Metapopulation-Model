...
process <- function(eta, theta_oak, theta_pine, gamma, SDa, SDe, SDs){
  N_P = 216
  N_O = 1587
  delta = 2.03
  alpha = 0.1
...
calcLogLik <- function(eta, theta_oak, theta_pine, gamma,SDa, SDe, SDs, summary, Z_P_M, Z_P_F, Z_O_M, Z_O_F){
  if (eta<0|theta_oak<0|theta_pine<0|gamma<0|SDa<0|SDe<0|SDs<0){
    loglik <- log(0)
  } else{
...
Metro_Hast <- function (eta0, theta_oak0, theta_pine0, gamma0, SDa0, SDe0, SDs0, N, Z_P_M, Z_P_F, Z_O_M, Z_O_F) {
...
SDs[1]=SDs0
lik=rep(NA, N)
lik[1]=calcLogLik(eta0, theta_oak0, theta_pine0, gamma0,SDa0, SDe0, SDs0, process(eta0, theta_oak0, theta_pine0, gamma0, SDa0, SDe0, SDs0), Z_P_M, Z_P_F, Z_O_M, Z_O_F)

prop.sd <- c(0.05, 0.5, 0.5, 0.01, 0.05, 0.05, 0.05)

  for (i in 2:N) {
    
    summary.old <- process(eta[i-1], theta_oak[i-1], theta_pine[i-1], gamma[i-1], SDa[i-1], SDe[i-1], SDs[i-1])
    ...
    theta.new <- c(eta.new, theta_oak.new, theta_pine.new, gamma.new, SDa.new, SDe.new, SDs.new)
    
    summary.new <- try(process(eta.new, theta_oak.new, theta_pine.new, gamma.new, SDa.new, SDe.new, SDs.new))
    if (class(summary.new) != "try-error") {
      loglik.new <- calcLogLik(eta.new, theta_oak.new, theta_pine.new, gamma.new,SDa.new,SDe.new,SDs.new,summary.new, Z_P_M, Z_P_F, Z_O_M, Z_O_F)
      loglik.old <- calcLogLik(eta[i-1], theta_oak[i-1], theta_pine[i-1], gamma[i-1],SDa[i-1], SDe[i-1], SDs[i-1], summary.old, Z_P_M, Z_P_F, Z_O_M, Z_O_F)
  ...
  return(list(eta, theta_oak, theta_pine, gamma, SDa, SDe, SDs, lik))
}


#simulate data
eta = runif(1, 0, 0.5)
theta_oak = runif(1, 21.57, 23.79)
theta_pine = runif(1, 24.67, 26.95)
gamma = runif(1, 0, 0.2)
SDa = runif(1, 0.5, 2)
SDe = runif(1, 0.2, 1.2)
SDs = runif(1, 0.2, 1.2)
sim_data <- process(eta, theta_oak, theta_pine, gamma, SDa, SDe, SDs)
Z_P_M <- sim_data[[9]]
Z_P_F <- sim_data[[10]]
Z_O_M <- sim_data[[11]]
Z_O_F <- sim_data[[12]]
...
results <- Metro_Hast(eta, theta_oak, theta_pine, gamma, SDa, SDe, SDs, N=10, Z_P_M, Z_P_F, Z_O_M, Z_O_F)



rows=10
names=c("eta", "theta_oak", "theta_pine", "gamma", "SDa", "SDe", "SDs")
...
X[,4] <- qunif(X[,4], 0, 0.2)
X[,5] <- qunif(X[,5], 0.5, 2)
X[,6] <- qunif(X[,6], 0.2, 1.2)
X[,7] <- qunif(X[,7], 0.2, 1.2)
params <- data.frame(X)
...
estimates <- data.frame(cbind(params, eta.mode=rep(NA, rows), theta_oak.mode=rep(NA, rows), theta_pine.mode=rep(NA, rows), 
                        gamma.mode=rep(NA, rows), SDa.mode=rep(NA, rows), SDe.mode=rep(NA, rows), SDs.mode=rep(NA, rows) ))
...
for (i in 1:nrow(estimates)) {
  ...
  sim_data <- try(with(estimates, process(eta[i], theta_oak[i], theta_pine[i], gamma[i], SDa[i], SDe[i], SDs[i])))
  ...
  if (class(sim_data) != "try-error") {
    ...
    Z_O_M <- sim_data[[11]]
    Z_O_F <- sim_data[[12]]
    results <-with(estimates, Metro_Hast(eta[i], theta_oak[i], theta_pine[i], gamma[i], SDa[i], SDe[i], SDs[i], 
                                       N=5, Z_P_M, Z_P_F, Z_O_M, Z_O_F))
    
    results.eta <- as.vector(window(mcmc(results[[1]]), start=first, end=rows, thin=thin))
    results.theta_oak <- as.vector(window(mcmc(results[[2]]), start=first, end=rows, thin=thin))
    results.theta_pine <- as.vector(window(mcmc(results[[3]]), start=first, end=rows, thin=thin))
    results.gamma <- as.vector(window(mcmc(results[[4]]), start=first, end=rows, thin=thin))
    results.SDa <- as.vector(window(mcmc(results[[5]]), start=first, end=rows, thin=thin))
    results.SDe <- as.vector(window(mcmc(results[[6]]), start=first, end=rows, thin=thin))
    results.SDs <- as.vector(window(mcmc(results[[7]]), start=first, end=rows, thin=thin))
