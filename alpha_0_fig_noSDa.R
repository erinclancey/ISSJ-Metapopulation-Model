...
rows <- processed[sample(nrow(processed), 375), ]
...
with(rows,process(eta[1], theta_oak[1], theta_pine[1], gamma[1], SDe[1], SDs[1], m[1]))
...
t_M = (mean(deltazbar$delta_M) - 0.794) / sd(deltazbar$delta_M)
2*pt(-abs(t_M),df=length(deltazbar$delta_M)-1)
t_F = (mean(deltazbar$delta_F) - 0.797) / sd(deltazbar$delta_F)
2*pt(-abs(t_F),df=length(deltazbar$delta_F)-1)
